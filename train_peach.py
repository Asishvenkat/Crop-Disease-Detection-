"""
Peach Disease Detection Training
...
MODEL_SAVE_PATH = PROJECT_ROOT / "models" / "pdd_peach.keras"
CLASS_MAPPING_PATH = PROJECT_ROOT / "models" / "class_mapping_peach.json"
...
def create_model(num_classes: int) -> Sequential:
    print(f"\n📦 Building MobileNetV2 model for {num_classes} peach classes...")
...
def train_model(model: Sequential, train_gen, val_gen):
    print(f"\n🚀 Training peach model for up to {EPOCHS} epochs...")
    callbacks = [
        ModelCheckpoint(
...
def evaluate_model(model: Sequential, val_gen):
    print("\n📊 Evaluating peach model on validation set...")
    validation_steps = max(1, val_gen.samples // BATCH_SIZE)
...
def main():
    print("=" * 70)
    print("🍑 PEACH DISEASE DETECTION MODEL TRAINING")
    print("=" * 70)
    ...
    print("\n" + "=" * 70)
    print("✅ PEACH MODEL TRAINING COMPLETE")
